...
export function initTools() {
  ...
  document.addEventListener("click", e => {
    if (
      colorOptionsPanel.classList.contains("visible") &&
    ...
    if (selectionStart === selectionEnd) {
      ...
      if (e.key === "Enter") {
        ...
        if (emptyNumberedMatch || emptyBulletMatch) {
          ...
          const indentation =
            emptyNumberedMatch?.[1] || emptyBulletMatch?.[1] || "";
          const newText =
            text.substring(0, lineStartPos) + indentation + textAfterCursor;
...
function initColorPalette(container) {
  ...
  COLORS.forEach(color => {
    ...
    if (color === getState().currentColor) {
...
export function activateTextEditor(x, y, existingObject = null) {
  ...
  if (existingObject) {
    ...
    const { fontSize: currentFontSize, fontFamily: fontName } = parseFont(
      existingObject.font
    );
...
export function deactivateTextEditor() {
  if (!textEditor || textEditor.style.visibility === "hidden") {
    return;
  }
...
export function selectTool(toolName) {
  ...
  document
    .querySelectorAll(".tool-button, .shape-button")
    .forEach(btn => btn.classList.remove("active"));

  const toolButtonId =
    toolName === "shape" ? "shapes-tool" : `${toolName}-tool`;
